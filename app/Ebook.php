...
class Ebook extends Model
{
    ...
    public static function getQueryByCondition(array $data, $orderBy = 'views', $user_id = null)
    {
        ...
        if ($data['type'] == 'library' && $user_id) {
            ...
            $qr->groupBy(['ebook.id', 'library.ebook_id', 'library.user_id']);
        } else {
    ...
    public static function getLinkApi($items)
    {
        foreach ($items as $key => $item) {
            if ($item->link_content) {
                $items[$key]->link_content = asset('upload/content/' . $item->link_content);
            }
            if ($item->image) {
                $items[$key]->image = asset('upload/ebook/' . $item->image);
            }
        }
